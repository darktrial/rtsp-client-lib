...
static int hls_coding_unit(HEVCContext *s, int x0, int y0, int log2_cb_size)
{
    ...
    for(int i = 0; i < weight; i++) {
        //printf("weight=%d qp=%i\n",weight, lc->qp_y);
        if (min_qp == 0 || min_qp > lc->qp_y)
        	min_qp=lc->qp_y;
        if (lc->qp_y > max_qp)
        	max_qp=lc->qp_y;
        sum_qp=sum_qp+lc->qp_y;
    }
    double avg_qp = (double)sum_qp / (weight);
    //printf("min_qp=%d max_qp=%d avg_qp=%.2f\n",min_qp,max_qp,avg_qp);
    global_min_qp=min_qp;
    global_max_qp=max_qp;
    global_avg_qp=avg_qp;
    return 0;
}
//modified by joey
int ffpg_get_minqp()
{
	return global_min_qp;
}

int ffpg_get_maxqp()
{
	return global_max_qp;
}

double ffpg_get_avgqp()
{
	return global_avg_qp;
}
